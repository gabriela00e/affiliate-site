...
// Explicit, uniform shape for every seed row. Only the columns that are
// genuinely optional in supabase/schema.sql (nullable, or backed by a
// database DEFAULT) are marked optional here — everything else is required,
// and every product object below fills in every key so the array stays a
// single consistent type instead of a union of per-object literal types.
type SeedProduct = {
  name: string;
  slug: string;
  ...
  const products: SeedProduct[] = [
    {
      ...
      long_description:
        "Formulated with 15% vitamin C and ferulic acid to fade dark spots and boost radiance over 4–6 weeks of daily use.",
